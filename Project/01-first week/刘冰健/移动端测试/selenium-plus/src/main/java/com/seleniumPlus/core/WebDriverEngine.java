...
public class WebDriverEngine {
    ...
    //locator参数为By类型
    //单位秒
    public WebElement waitElement(By locator,int sec) throws Exception{
        try {
    ...
    //通过js点击页面元素
    //name,id,class 三种定位方法
    public void jsClick(String locator) {
        String target = locator.trim();
        if(target.startsWith("id=")){
            locator = locator.substring("id=".length());
            runJs("document.getElementById(\"" + locator +"\").click()");
        }else if(target.startsWith("class=")){
            locator = locator.substring("class=".length());
            runJs("document.getElementByClass(\"" + locator +"\").click()");
        }else if(target.startsWith("name=")) {
            locator = locator.substring("name=".length());
            runJs("document.getElementByName(\"" + locator +"\").click()");
        }
    }

    //设置元素属性值
    public void setAttributeValue(String locator,String attributeName,String value) {
        String target = locator.trim();
        if(target.startsWith("id=")){
            locator = locator.substring("id=".length());
            runJs("document.getElementById(\"" + locator +"\").setAttribute(\""+attributeName+"\",\""+value+"\")");
        }else if(target.startsWith("class=")){
            locator = locator.substring("class=".length());
            runJs("document.getElementByClass(\"" + locator +"\").setAttribute(\""+attributeName+"\",\""+value+"\")");
        }else if(target.startsWith("name=")) {
            locator = locator.substring("name=".length());
            runJs("document.getElementByName(\"" + locator +"\").setAttribute(\""+attributeName+"\",\""+value+"\")");
        }
    ...
    //获取节点数量
    public int getNum(String locator){
        String target = locator.trim();
        if (target.startsWith("id=")) {
            locator = locator.substring("id=".length());
            ...
            return driver.findElements(By.linkText(locator)).size();
        } else if (target.startsWith("partLink=")) {
            locator = locator.substring("partLink=".length());
            return driver.findElements(By.partialLinkText(locator)).size();
            ...
            return driver.findElements(By.tagName(locator)).size();
        } else {
